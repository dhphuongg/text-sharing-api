...
const handlePrismaError = (err) => {
  switch (err.code) {
    case 'P2002':
      return new ApiError(httpStatus.BAD_REQUEST, `Duplicate field value: ${err.meta.target}`);
    case 'P2014':
      return new ApiError(httpStatus.BAD_REQUEST, `Invalid ID: ${err.meta.target}`);
    case 'P2003':
      return new ApiError(httpStatus.BAD_REQUEST, `Invalid input data: ${err.meta.target}`);
    default:
      return new ApiError(httpStatus.INTERNAL_SERVER_ERROR, `Something went wrong: ${err.message}`);
  }
};
